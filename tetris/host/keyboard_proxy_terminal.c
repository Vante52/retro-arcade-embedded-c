...
void keyboard_proxy_terminal_init(keyboard_proxy_terminal_t *proxy) {
    ...
    if (proxy == NULL || proxy->initialized) {
        return;
    }

    if (tcgetattr(STDIN_FILENO, &proxy->tty) != 0) {
        return;
    }
...
keyboard_key_t keyboard_proxy_terminal_read(keyboard_proxy_terminal_t *proxy) {
    ...
    if (!read_byte_nonblocking(&ch)) {
        return KEYBOARD_KEY_NONE;
    }

    if (ch == 27u) {
        if (!read_byte_nonblocking(&next1)) {
        ...
        if (!read_byte_nonblocking(&next2)) {
            return KEYBOARD_KEY_UNKNOWN;
        }
